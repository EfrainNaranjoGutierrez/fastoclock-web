...
function PredictionsContent() {
  ...
  const download = () => window.open(`${API}/api/predict/${jobId}/download`, '_blank');
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="border-b bg-white px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={download} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700">⬇ Descargar CSV</button>
        </div>
      </nav>
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-3 mb-6 flex-wrap items-center">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar cliente..."
            className="flex-1 min-w-[250px] px-4 py-2.5 border rounded-xl text-sm" />
          <div className="flex gap-1">
            {['prob_7d','prob_14d','prob_30d','prob_60d'].map(h => (
              <button key={h} onClick={() => setHorizon(h)}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition ${horizon === h ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((p: any, i: number) => (
                  <tr key={i} className={`border-b hover:bg-gray-50 cursor-pointer ${selected?.cliente === p.cliente ? 'bg-emerald-50' : ''}`}
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${p.prioridad === 'ALTA' ? 'bg-emerald-100 text-emerald-700' : p.prioridad === 'MEDIA' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{p.segmento}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width:`${p[horizon]*100}%`, background: p[horizon]>0.75?'#059669':p[horizon]>0.4?'#d97706':'#dc2626' }}/>
                        </div>
                        <span className="font-mono text-xs font-bold">{(p[horizon]*100).toFixed(0)}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3"><span className={`text-lg font-extrabold ${p.dias_estimados<=7?'text-emerald-600':p.dias_estimados<=21?'text-amber-600':'text-red-600'}`}>{p.dias_estimados}</span><span className="text-[10px] text-gray-400 ml-0.5">d</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <div className="w-10 h-1.5 bg-gray-100 rounded overflow-hidden"><div className="h-full rounded" style={{ width:`${p.fill_rate}%`, background: p.fill_rate>90?'#059669':p.fill_rate>70?'#d97706':'#dc2626' }}/></div>
                    ...
                    <td className="px-4 py-3">
                      <button className={`px-3 py-1.5 rounded-lg text-[10px] font-bold ${p.prioridad==='ALTA'?'bg-emerald-100 text-emerald-700':p.prioridad==='MEDIA'?'bg-amber-100 text-amber-700':'bg-red-100 text-red-700'}`}>
          ...
          <div className="mt-6 bg-white rounded-2xl border-2 border-emerald-200 p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${selected.prioridad==='ALTA'?'bg-emerald-100 text-emerald-700':selected.prioridad==='MEDIA'?'bg-amber-100 text-amber-700':'bg-red-100 text-red-700'}`}>{selected.segmento}</span>
              </div>
            ...
            <div className="bg-emerald-50 rounded-xl p-4 mb-4">
              <p className="text-sm text-emerald-900 leading-relaxed">📋 <strong>Interpretación:</strong> {selected.interpretacion}</p>
            </div>
              ...
              {['7d','14d','30d','60d'].map(h => {
                ...
                return (
                  <div key={h} className="text-center">
                    ...
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width:`${val*100}%`, background: val>0.75?'#059669':val>0.4?'#d97706':'#dc2626' }}/></div>
            ...
            <div className="grid grid-cols-5 gap-3">
              {[['Órdenes',selected.total_ordenes],['Ticket','$'+selected.ticket_promedio?.toLocaleString()],['Revenue','$'+selected.revenue_total?.toLocaleString()],['Intervalo',selected.intervalo_dias+'d'],['Fill Rate',selected.fill_rate+'%']].map(([l,v])=>(
